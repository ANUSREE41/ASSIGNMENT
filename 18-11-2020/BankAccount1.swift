...
class Bank {
   ...
   func processRequest() {
        ...
        let keyArray = accountDetailsList.keys
           ...
           requestloop: for entry in requests {
                 ...
                 if (keyArray.contains(entry.accountNumber)) {
                    ...
                    if let temp = accountDetailsList[entry.accountNumber] {
                        ...
                        let account = temp
                            ...
                            managers.insert(managers[0] ,at: managers.count)
                                 managers.remove(at: 0)
...
class Account: BankProtocolForAccount, AccountManagerReceiveProtocol {
    ...
    func accountNumber(number: String) {
     ...
     func accountDetails() -> String {
         let details = "accountHolderName: \(accountHolderName) , accountNumber: \(accountNumber)" 
         return details 
     }
...
var accountManager2 = AccountManager()
var accountManager3 = AccountManager()
...
bank1.addAccountManager(manager: accountManager2)
bank1.addAccountManager(manager: accountManager3)
